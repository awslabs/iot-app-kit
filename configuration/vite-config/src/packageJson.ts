...
export function readPackageJson(packageJsonPath: string): PackageJson {
  console.info(
    `*** Reading package.json file from path: ${packageJsonPath} ***`
  );
...
function isValidPackageJson(
  maybePackageJson: PackageJson | unknown
): maybePackageJson is PackageJson {
  const asPackageJson = maybePackageJson as PackageJson;
