...
Refer to the [AWS-UI/table](https://github.com/aws/awsui-documentation/blob/main/components/table.md) for all available properties.  The properties with differences 
...
#### $cellRef (object, optional):
$cellRef is a keyword in `Item`. We use it to find the corresponding dataStream. In the above example,
the item has two properties. Both use `$cellRef` to map properties to the data stream.

  - `id` (string, optional): A string representing the id of a time series data stream. which you can get by utilizing the time series data source of choices `toId` helper, 
  for example, with AWS IoT SiteWise, you can refer to an id with the following code:
...
#### propertyFiltering (object, optional):
...
- `filteringProperties` (array): An array of properties by which the data set is filtered. Individual items have the following properties:
  -`key` (string): The identifier of this property. Refers to a property name in type T of the data item to enable built-in filtering.
  - `groupValuesLabel` (string): Localized string that displays the 'Values' group label for a specific property. For example, EC2 instance values.
  - `propertyLabel` (string): A human-readable string value for the property.
  - `operators` (optional): A list of all operators supported by this property. Equals operator must be supported, even if omitted in the list.
  - `group` (string, optional): Optional identifier of a custom group that the filtering option is assigned to. This is used to create additional groups 
    below the default one. Labels for the group in the customGroupsText property must be defined. Only one level of nesting options is supported.
...
the default viewport is `{ duration: '10d' }`. Learn about the [viewport](/docs/core-viewport--docs).
