...
**viewport** (object, optional): This specifies the interval of time the widget visualizes. If not present, the widget utilizes the `viewport` specified by its [ViewportManager](/docs/components-viewportmanager--docs).
...
**thresholds** (optional): A styled threshold defines rules that effect how the data is visualized. It's based on if the data breaches the threshold as well as how the threshold is styled.
...
**styles** (object, optional): This contains a map of `refId` to style settings for the component. [Learn more about reference IDs here](/docs/core-styles--docs).
