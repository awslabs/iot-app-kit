...
**assetId** (string): The ID of the asset for which you want to fetch anomaly data.

**predictionDefinitionId** (string): The ID of the prediction definition associated with the anomaly data.

**startDate** (number, optional): The start date for the anomaly data in milliseconds since the Unix epoch. If not provided, the query will fetch data from the earliest available timestamp.

**endDate** (number, optional): The end date for the anomaly data in milliseconds since the Unix epoch. If not provided, the query will fetch data up to the latest available timestamp.

**interval** (number, optional): The interval in milliseconds at which to fetch anomaly data. If not provided, the query will use the default interval.
