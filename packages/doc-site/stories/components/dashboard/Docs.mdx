...
**clientConfiguration** (object): This gives the configuration information for the AWS SDK Clients required to utilize AWS IoT SiteWise data within the dashboard. This contains the following properties:
...
**onSave** (function, optional): The function called when the dashboard is saved. Expected to return promise which resolves when the saving is complete. A rejected promise represents a failure to save. The function must be of the following type:
- `(dashboardConfiguartion: DashboardConfiguration) => void`.

**currentViewport** (object, optional): If provided, this viewport represents the viewport all charts should render. To be used alongside `onViewportChange` when users manage their own dashbaord viewports. 

**onViewportChange** (function, optional): Callback that is called when the viewport changes. This includes when a user gestures on a widget (zooms in, zooms out, and scrolls). The function must be of the following type:
...
**initialViewMode** ('preview' | 'edit', optional): represents the initial viewmode which the dashboard loads.

**name** (string, optional): Specifies the dashboard name visible in the default toolbar.

**toolbar** (function, optional): This renders a function that replaces the defaul dashboard toolbar with a user-provided component. The render function is of the following type:
- `({dashboardConfiguartion: DashboardConfiguration, viewport?: Viewport, viewmode: 'preview' | 'edit'}) => ReactElement`.

**timeZone** (string, optional): Timezone must be specified as an [IANA timezone string](https://ftp.iana.org/tz/tzdb-2020f/zone1970.tab). If this property is provided, the dashboard will display all dates in the provided timezone, otherwise it will use the browsers local timezone. **Note:** Bar chart and Timeline charts are not currently supported.

**onDashboardConfigurationChange** (function, optional): Callback called when the dashboard configuration is updated. This is called when a widget is added, deleted, updated, or moved. 
The function must of the following type:
- `(dashboardConfiguartion: DashboardConfiguration) => void`.

**dashboardConfiguration** (object): This specifies the widgets contained within the dashboard, the configuration of those widgets, the data is displayed within the widgets, the default viewport of the dashboard
and the display configurations for the dashboard. Contains the following properties:
