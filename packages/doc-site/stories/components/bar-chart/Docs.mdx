...
**axis** (object, optional): Specifies x and y-axis related settings. Defaults to `{ showX: true, showY: true, yAxisLabel: undefined }`. It has the following properties:
...
**styles** (object, optional): The styles prop lets you customize the appearance of each data stream based on its `refId` or [reference ID](/docs/core-styles--docs). It has the following properties:
