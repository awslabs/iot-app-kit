...
**viewport** (object, optional): This specifies the time interval that the widget visualizes. If not provided, the widget
    utilizes the `viewport` specified by its [ViewportManager](/docs/components-viewportmanager--docs). If a
    ViewportManager, and `viewport` is not defined, the default viewport is `{ duration: '10s' }`. [Learn about the viewport](/docs/core-viewport--docs).

**thresholds** (optional): This specifies rules that effect how the data is visualized based on if the data breaches the threshold. [Learn more about thresholds](/docs/core-thresholds--docs#thresholds).

**thresholdSettings** (object, optional): This customizes the threshold visualization settings in the widget. 
    Default value is `{ colorBreachedData: true }`. [Learn more about thresholds settings](/docs/core-thresholds--docs#thresholdsettings).
 
**aggregationType** (string, optional): This specifies the data aggregation window. Default value is `undefined`.

**gestures** (optional): This specifies if gestures (pan, zoom and restrict time-span) are enabled. Default value is `true`.

**axis** (optional): This specifies x and y-axis related settings. Default value is `{ showX: true, showY: true, yAxisLabel: undefined }`. It has the following properties:
...
**backgroundColor** (string, optional): This specifies the background color of the chart. Default value is `undefined`.

**fontSettings** (optional): This specifies the chart's font color and font size. Default value is `undefined`. It has the following properties:
...
**significantDigits** (number, optional): This specifies the number of significant digits displayed. Default value is `4`.

**theme** (string, optional): This specifies the light or dark theme used. The type must be one of `'light' | 'dark'`.
 Default value is `light`. 

**id** (string, optional): This specifies an **unique** id for the chart. Default value is `undefined`.

**titleText** (string, optional): This specifies the chart title. Default value is `undefined`.

**legend** (optional): This specifies the color and position of the legend section. Default value is `undefined`. It has the following properties:

    * `backgroundColor` (string, optional): This specifies the background color of the legend section. 
    It can be either a color name or its hex value. Default value is `undefined`.
    * `position[WIP]` (string, optional): This pecifies the position of the legend in the chart. The type must be 
    one of `'top' | 'bottom' | 'left', 'right'`. Default vallue is `'right'`.
