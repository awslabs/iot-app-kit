import { getUrlExtension } from '3d-tiles-renderer/src/utilities/urlExtension.js';
import { LRUCache } from '3d-tiles-renderer/src/utilities/LRUCache.js';
import { PriorityQueue } from '3d-tiles-renderer/src/utilities/PriorityQueue.js';
import {
  determineFrustumSet,
  toggleTiles,
  skipTraversal,
  markUsedSetLeaves,
  traverseSet,
} from '3d-tiles-renderer/src/base/traverseFunctions.js';
import { UNLOADED, LOADING, PARSING, LOADED, FAILED } from '3d-tiles-renderer/src/base/constants.js';
import { TilesRendererBase as BaseTilesRendererBase } from '3d-tiles-renderer';
...
export class TilesRendererBase extends BaseTilesRendererBase {
  get rootTileSet() {
  ...
  loadRootTileSet(url) {
    const tileSets = this.tileSets;
    if (!(url in tileSets)) {
      const pr = this.fetchTileSet(this.preprocessURL ? this.preprocessURL(url) : url, this.fetchOptions).then(
        (json) => {
          tileSets[url] = json;
        },
      );
    ...
    if (isExternalTileSet) {
      ...
      downloadQueue
        .add(tile, (downloadTile) => {
          ...
          return this.fetch(uri, Object.assign({ signal }, this.fetchOptions));
        })
        .then((res) => {
          if (tile.__loadIndex !== loadIndex) {
            return;
