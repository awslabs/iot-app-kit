...
import { urlJoin } from '3d-tiles-renderer/src/utilities/urlJoin';
...
import { determineFrustumSet, toggleTiles, skipTraversal, markUsedSetLeaves, traverseSet } from '3d-tiles-renderer/src/base/traverseFunctions';
...
export class TilesRendererBase {
	...
	constructor( url, twinMakerFetch ) {
		...
		this.twinMakerFetch = twinMakerFetch;
		// - Amazon --------
	...
	fetchTileSet( url, fetchOptions, parent = null ) {
		...
		// + Amazon --------
		const fetchFunc = this.twinMakerFetch ?? fetch;
	...
	requestTileContents( tile ) {
		...
		if ( isExternalTileSet ) {
			...
			downloadQueue.add( tile, downloadTile => {
				...
				// + Amazon --------
				return fetchFunc( uri, Object.assign( { signal }, this.fetchOptions ) );
				// - Amazon --------
