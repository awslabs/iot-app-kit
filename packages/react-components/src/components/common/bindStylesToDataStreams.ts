...
const assignDefaultColor = ({
  ...
  const associatedStyles = dataStream.id != null && styleSettings != null ? styleSettings[dataStream.id] : {};
...
export const bindStylesToDataStreams = ({
  ...
  const streams = dataStreams.map((dataStream) =>
    styleSettings == null || dataStream.id == null
      ? dataStream
      : {
          ...dataStream,
          ...styleSettings[dataStream.id],
        }
  );
