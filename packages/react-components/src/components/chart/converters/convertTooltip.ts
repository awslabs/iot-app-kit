...
export const convertTooltip = (significantDigits: ChartOptions['significantDigits']): TooltipComponentOption => ({
  ...DEFAULT_TOOLTIP,
  valueFormatter: (value) => {
    if (Array.isArray(value)) return value.map(formatValue(significantDigits)).join(', ');
