...
export const calculateSyncDelta = ({ syncedTrendCursors = {}, graphic }: SyncChanges) => {
  const existingIds: string[] = graphic.map((g) => g.id as string);
  const toBeAdded = Object.keys(syncedTrendCursors).filter(
    (syncedTrendCursorId) => existingIds.findIndex((id) => id === syncedTrendCursorId) === -1
  );
  ...
  const toBeUpdated = graphic
    ...
    .filter((g) => {
      const syncedTrendCursorTimeStamp = syncedTrendCursors[g.id as string]?.timestamp;
      return !!(syncedTrendCursorTimeStamp && syncedTrendCursorTimeStamp !== g.timestampInMs);
    });
