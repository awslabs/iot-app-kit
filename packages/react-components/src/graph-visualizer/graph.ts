import cytoscape, { use, EventObjectCore, EventObjectEdge } from 'cytoscape';
import type {
  Css,
  EdgeDefinition,
  ElementsDefinition,
...
export function createGraph(
  ...
  ): NodeRenderData | EdgeRenderData | undefined {
...
function getNodeStyle({
  ...
  const labelProps: Css.Node = {
    ...
    'text-background-opacity': labelBackgroundColor ? 0 : 1,
  ...
  const getPolygonPointsStr = (node: NodeSingular) => {
    const result = getPolygonPoints(node);
  ...
  return {
    ...
    'overlay-opacity': 0,
    shape: shape ?? 'ellipse',
    'shape-polygon-points': getPolygonPointsStr,
