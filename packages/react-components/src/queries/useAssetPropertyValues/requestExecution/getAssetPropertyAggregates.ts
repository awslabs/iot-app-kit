    ...
    >
{
  ...
  private handleError(
    ...
    console.error(
      `Failed to get asset property aggregates. Error: ${error}`
    );
  ...
  public async send(
    ...
    try {
      do {
        ...
        const reducedAssetPropertyAggregateValues = takeRight(assetPropertyAggregateValues, dataPointNumberTarget);
        dataPointNumberTarget -= assetPropertyAggregateValues.length;

        const datapoints = createNonNullableList(reducedAssetPropertyAggregateValues.map(aggregateToDataPoint));
