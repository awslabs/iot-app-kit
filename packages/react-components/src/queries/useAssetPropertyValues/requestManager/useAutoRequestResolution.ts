...
import { IntervalTransformer, Viewport, getViewportType } from "../../useTimeSeriesData";
import { AssetPropertyAggregatesRequest, AssetPropertyValueHistoryRequest, AssetPropertyValuesRequest } from "../types";
import { AggregateType } from "@aws-sdk/client-iotsitewise";
import { useMemo } from "react";

const hasResolution = (request: AssetPropertyValuesRequest): request is AssetPropertyAggregatesRequest => {
  return 'resolution' in request && request.resolution != null;
}

const hasAggregates = (request: AssetPropertyValuesRequest): request is AssetPropertyAggregatesRequest => {
...
export const useAutoRequestResolution = ({ requests, viewport, resolutionConfigurations = DEFAULT_RESOLUTION_CONFIGURATIONS }: UseAutoRequestResolutionOptions) => {
  return useMemo(() => {
    const sortedResolutionConfgurations = resolutionConfigurations.sort((a, b) => a.duration - b.duration);
    ...
    return requests.map(request => {
      ...
      const resolutionConfiguration = sortedResolutionConfgurations.find(configuration => {
        return intervalDuration <= configuration.duration;
      });
 
      const resolution = resolutionConfiguration?.resolution;
      const aggregateTypes = hasAggregates(request) ? request.aggregateTypes :  [AggregateType.AVERAGE];
