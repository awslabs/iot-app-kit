...
export const getVisibleData = <T extends Primitive>(
  ...
  const start: Date = isMinimalStaticViewport(viewport)
    ? new Date((viewport as MinimalStaticViewport).start)
    : new Date(Date.now() - parseDuration((viewport as MinimalLiveViewport).duration));
  const end: Date = isMinimalStaticViewport(viewport) ? new Date((viewport as MinimalStaticViewport).end) : new Date();
