...
import { CHART_RESIZE_INITIAL_FACTOR, CHART_RESIZE_MAX_FACTOR, CHART_RESIZE_MIN_FACTOR } from '../../components/chart/eChartsConstants';
 ...
 */
export const useResizeableEChart = (chartRef: React.MutableRefObject<ECharts | null>, size: { width: number; height: number }) => {
  const { width, height } = size;
  const [chartWidth, setWidth] = useState(width * CHART_RESIZE_INITIAL_FACTOR);
