...
export type AlarmData = {
   ...
   * Currently only a single source property is supported in a SiteWise and Events alarm.
   *
  ...
  state: {
    /**
     * The state of an alarm modeled as a complex data type.
     */
  ...
  source?: {
    /**
     * The Events alarm model source. Only supported for "IOT_EVENTS" alarm type.
     */
    property: AssetProperty;
    data?: AssetPropertyValue[];
  };
...
export type AlarmCompositeModelRequest = {
  ...
  sourcePropertyId?: never;
  assetModelId?: never;
};

/**
 * Request data for all alarms that use a source asset property
 ...
 */
export type AlarmSourcePropertyRequest = {
  assetId: string;
  sourcePropertyId: string;
 ...
 */
export interface UseAlarmsHookProps<T> {
