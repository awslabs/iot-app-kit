...
type CloudscapePreferences = NonNullable<
  CloudscapeCollectionPreferencesProps['preferences']
>;

function userCustomizationToCloudscapePreferences(
  userCustomization: UserCustomization
): CloudscapePreferences {
  const cloudscapePreferences = {
    ...userCustomization,
    contentDisplay: userCustomization.columnDisplay.map(
      ({ id, isVisible: visible }) => ({
        id,
        visible,
      })
    ),
  } satisfies CloudscapePreferences;

  return cloudscapePreferences;
}
...
export function ResourceTableUserSettings<Resource>({
  ...
  const cloudscapePreferences =
    userCustomizationToCloudscapePreferences(userCustomization);

  return (
    <CloudscapeCollectionPreferences
      preferences={cloudscapePreferences}
      onConfirm={({ detail }) => {
        const settings = {
          ...detail,
          columnDisplay:
