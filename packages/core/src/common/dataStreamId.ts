...
export const toDataStreamId = ({ assetId, propertyId }: { assetId: string; propertyId: string }): string =>
  `${assetId}${ID_SEPARATOR}${propertyId}`;

export const toSiteWiseAssetProperty = (dataStreamId: string): { assetId: string; propertyId: string } => {
