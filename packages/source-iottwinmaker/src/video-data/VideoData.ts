...
export class VideoDataImpl implements VideoData {
  ...
  private timerangesWithSource: Array<{
    start: number;
    end: number;
    src: string;
  }> = [];
  ...
  constructor(input: VideoDataInput) {
    ...
    this.kinesisVideoArchivedMediaClient =
      input.kinesisVideoArchivedMediaClient;
  ...
  async triggerLiveVideoUpload(): Promise<void> {
    if (
      ...
      await triggerVideoUploadRequest(
        liveVideoUploadRequest,
        this.siteWiseClient
      );
    }
  }

  async triggerOnDemandVideoUploadRequest(
  ...
  async getKvsStreamSrc(
    ...
    if (this.workspaceId && this.entityId && this.componentName) {
      ...
      if (
        !!this.kvsStreamNameCalculated &&
        !isEmpty(this.kvsStreamNameCalculated)
      ) {
        const videoSource = await this.getKVSSource(
          playbackMode,
          this.kvsStreamNameCalculated,
          startTime,
          endTime
  ...
  private async updateSiteWatchAndKvsProperties() {
    ...
    if (this.workspaceId && this.entityId && this.componentName) {
      const getEntityRequest: GetEntityCommandInput = {
        workspaceId: this.workspaceId,
        entityId: this.entityId,
      };
      const entity = await this.twinMakerClient.send(
        new GetEntityCommand(getEntityRequest)
      );
      if (entity && entity.components) {
        ...
        if (videoComponent && videoComponent.properties) {
          if (videoComponent.componentTypeId === edgeVideoComponentTypeId) {
            this.sitewiseAssetPropertyId = videoComponent.properties[
              sitewiseAssetId
            ].value?.stringValue as string;
            this.videoUploadedTimeRangePropertyId = videoComponent.properties[
              VideoUploadedTimeRange
            ].definition?.configuration?.sitewisePropertyId as string;
            this.videoUploadRequestPropertyId = videoComponent.properties[
              VideoUploadRequest
            ].definition?.configuration?.sitewisePropertyId as string;
            this.KinesisVideoStreamNamePropertyId = videoComponent.properties[
              KinesisVideoStreamName
            ].definition?.configuration?.sitewisePropertyId as string;
            ...
            const propertyValue = await getAssetPropertyValue(
            ...
            videoComponent.componentTypeId === kvsVideoComponentTypeId
          ) {
            // For KVS Component Type - Fetch the KVS streamm name and proceed further
            this.kvsStreamNameCalculated = videoComponent.properties[
              KinesisVideoStreamName
            ].value?.stringValue as string;
  ...
  > {
    if (
      ...
      const cachedVideoAgeOutOnEdgeHistory =
        await this.getSinglePropertyValueHistory(
          CachedVideoAgeOutOnEdge,
          startDateTime,
          endTime
        );
      const videoOnEdgeDataId = await createPropertyIndentifierKey(
        entityId,
        componentName,
        CachedVideoAgeOutOnEdge
      );
      cachedVideoAgeOutOnEdgeHistory[videoOnEdgeDataId].forEach((data) => {
        const start =
          data.x < startTime.getTime() ? startTime.getTime() : data.x;
        const end =
          Number(data.y) * 1000 > endTime.getTime()
            ? endTime.getTime()
            : Number(data.y) * 1000;
      ...
      const prevEnd = await getLastValueBeforeTimestamp(
        getLastValueBeforeTimestampRequest,
        this.siteWiseClient
      );
      ...
      const videoUploadedTimeRangeHistory =
        await this.getSinglePropertyValueHistory(
          VideoUploadedTimeRange,
          startDateTime,
          endTime
  ...
  private async getSinglePropertyValueHistory(
    propertyName: string,
    startDateTime: Date,
    endDateTime: Date
  ) {
    ...
    return await getSinglePropertyValueHistory(
  ...
  private async insertVideoInformation(start: Date, end: Date) {
    if (this.kvsEndpoint && !isEmpty(this.kvsStreamNameCalculated)) {
      ...
      if (!!videoURL && !isEmpty(videoURL)) {
        this.timerangesWithSource.push({
          start: start.getTime(),
          end: end.getTime(),
          src: videoURL,
        });
