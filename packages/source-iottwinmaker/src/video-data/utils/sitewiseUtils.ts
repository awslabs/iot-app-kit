...
export const triggerVideoUploadRequest = async (
  ...
  let propertyValue = '';
  if (
    triggerVideoUploadRequest.startTimestamp &&
    triggerVideoUploadRequest.endTimestamp
  ) {
  ...
  const response = await sitewiseClient.send(
    new BatchPutAssetPropertyValueCommand({ entries: [entry] })
  );
...
export const getAssetPropertyValue = async (
  ...
  const response = await sitewiseClient.send(
    new GetAssetPropertyValueCommand(req)
  );
...
export const getLastValueBeforeTimestamp = async (
  ...
  const timeInSeconds = Math.floor(
    getLastValueBeforeTimestampRequest.timestamp.getTime() / 1000
  );
  ...
  do {
    const response = await sitewiseClient.send(
      new GetInterpolatedAssetPropertyValuesCommand(req)
    );
  ...
  if (
    result.length > 0 &&
    result[0].value?.doubleValue !== undefined &&
    result[0].value?.doubleValue > timeInSeconds
  ) {
