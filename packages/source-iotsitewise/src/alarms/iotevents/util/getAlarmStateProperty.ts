...
const isAlarmState = (property: AssetModelProperty) =>
...
const getAlarmStatePropertyFromAssetModel =
  ...
  ({
    properties,
    name,
  }: AssetModelCompositeModel): AssetModelProperty | undefined => {
    const alarmStateProperty = properties?.find(
      (property) =>
        property?.id === alarmStatePropertyId && isAlarmState(property)
    );
