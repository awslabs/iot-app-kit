...
import { constructAlarmStream } from './constructAlarmStream';
import { toDataType } from '../../../time-series-data/util/toDataType';
import { ALARM_STATUS } from '../constants';

/**
 * infer if stream is an AWS IoT Events alarm stream ingested into AWS SiteWise Model Alarm State Property
 */
  ...
  if (!assetModel) {
    if (isIoTEventsAlarmStateProperty(dataStream.data[dataStream.data?.length - 1]?.y)) {
      return {
        ...dataStream,
        streamType: 'ALARM',
        data: constructAlarmStream({ data: dataStream.data }),
      } as DataStream;
    }
