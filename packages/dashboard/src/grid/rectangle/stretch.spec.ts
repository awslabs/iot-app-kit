...
import { resizeSelectionBox } from './stretch';
...
it('should resize selection box on top anchor', () => {
  ...
  expect(
    resizeSelectionBox({ selectionBox: curr, anchor, vector, grid })
  ).toEqual(expected);
});
...
describe('should not vertically resize selection box below minimum width', () => {
  ...
  leftAnchors.forEach((anchor) => {
    ...
    it(`should not resize selection box on ${anchor} anchor`, () => {
      expect(
        resizeSelectionBox({ selectionBox: curr, anchor, vector, grid }).width
      ).toEqual(1);
    });
  });
...
describe('should not horizontally resize selection box below minimum height', () => {
  ...
  topAnchors.forEach((anchor) => {
    ...
    it(`should not resize selection box on ${anchor} anchor`, () => {
      expect(
        resizeSelectionBox({ selectionBox: curr, anchor, vector, grid }).height
      ).toEqual(1);
    });
  });
...
const withinGrid = (newRect: Rectangle, gridRect: Rectangle) => {
  ...
  return (
    x >= gridX &&
    y >= gridY &&
    x + width <= gridX + gridWidth &&
    y + height <= gridY + gridHeight
  );
};
describe('should not resize selection box beyond grid', () => {
  ...
  type TestCase = {
    anchor: Anchor;
    startingPoint: (typeof startingPoints)[number];
    vector: (typeof vectors)[number];
  };
  const table: TestCase[] = anchors.flatMap((anchor) =>
    startingPoints.flatMap((startingPoint) =>
      vectors.map((vector) => ({ anchor, startingPoint, vector }))
    )
  );

  it.concurrent.each(table)(
    'on $anchor from $startingPoint with $vector',
    ({ anchor, startingPoint, vector }) => {
      const curr = {
        x: startingPoint.x,
        y: startingPoint.y,
        width: 10,
      ...
      const newRect = resizeSelectionBox({
        selectionBox: curr,
        anchor,
        vector,
        grid,
